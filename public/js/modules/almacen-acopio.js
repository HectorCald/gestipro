...
export async function mostrarAlmacenAcopio() {
    ...
    function calcularPesoTotal(pesoString) {
        return pesoString.split(';')
    ...
    const registrationHTML = `  
        ...
        const pesoTotal = producto.bruto.split(';')
            .reduce((total, lote) => total + parseFloat(lote.split('-')[0]), 0);
            ...
            try {
                ...
                if (data.success) {
                    ...
                    await registrarHistorial(
                        `${usuarioInfo.nombre} ${usuarioInfo.apellido}`,
                        'Edición',
                        `Motivo: ${motivo} - Producto: ${productoNombre} (${registroId})`
                    );
                    mostrarNotificacion({
